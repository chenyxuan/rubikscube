...
export default class Solver {
  ...
  solve(facelets: string): string {
    ...
    if (verify.length > 0) {//不合法性 + 原因
      return "error: " + verify;
    }
 
    this.sol = 22;
    this.moveSol = null;
    this.initSearch();//初始搜索结果
    const solution = this.search();//后来的搜索结果
    return solution;
  }

  private conjMask: number;
  private initSearch(): void {
    this.conjMask = (Solver.TRY_INVERSE ? 0 : 0x38) | (Solver.TRY_THREE_AXES ? 0 : 0x36);//three axes三维坐标 56 54 按位取或
    this.maxPreMoves = this.conjMask > 7 ? 0 : Solver.MAX_PRE_MOVES;//最大pre步骤

    for (let i = 0; i < 6; i++) {
      this.urfCubieCube[i].copy(this.cc);//copy当前魔方状态
      this.urfCoordCube[i].setWithPrun(this.urfCubieCube[i], 20);//魔方状态和搜索深度
  ...
  private search(): string {
    for (this.length1 = 0; this.length1 < this.sol; this.length1++) {
      for (this.urfIdx = 0; this.urfIdx < 6; this.urfIdx++) {
        if ((this.conjMask & (1 << this.urfIdx)) != 0) {//1左移urfidx位
          continue;
        }
        if (this.phase1PreMoves(this.maxPreMoves, -30, this.urfCubieCube[this.urfIdx]) == 0) {
          return this.moveSol == null ? "error: no solution for prob" : this.getSolution();//没有可能解
        }
      }
    }
    return this.moveSol == null ? "error: no solution for depth" : this.getSolution();//超过预定深度
  }

  private getSolution(): string {//操作步骤的展示
    let ret = "";
    if (!this.moveSol) {
      return ret;
    }
    ...
    if (urf < 3) {
      for (let s = 0; s < this.moveSol.length; ++s) {
        ret += Util.MOVE2STR[CubieCube.URFMove[urf][this.moveSol[s]]] + " ";//某个操作步骤
      }
    } else {
