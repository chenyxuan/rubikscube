...
export default class Solver {
  ...
  */
  ...
  private initSearch(): void {
    this.conjMask = (Solver.TRY_INVERSE ? 0 : 0x38) | (Solver.TRY_THREE_AXES ? 0 : 0x36);//three axes��ά���� 56 54 ��λȡ��
    this.maxPreMoves = this.conjMask > 7 ? 0 : Solver.MAX_PRE_MOVES;//���pre����

    for (let i = 0; i < 6; i++) {
      this.urfCubieCube[i].copy(this.cc);//copy��ǰħ��״̬
      this.urfCoordCube[i].setWithPrun(this.urfCubieCube[i], 20);//ħ��״̬���������
  ...
  private search(): string {
    for (this.length1 = 0; this.length1 < this.sol; this.length1++) {
      for (this.urfIdx = 0; this.urfIdx < 6; this.urfIdx++) {
        if ((this.conjMask & (1 << this.urfIdx)) != 0) {//1����urfidxλ
          continue;
        }
        if (this.phase1PreMoves(this.maxPreMoves, -30, this.urfCubieCube[this.urfIdx]) == 0) {
          return this.moveSol == null ? "error: no solution for prob" : this.getSolution();//û�п��ܽ�
        }
      }
    }
    return this.moveSol == null ? "error: no solution for depth" : this.getSolution();//����Ԥ�����
  }

  private getSolution(): string {//���������չʾ
    let ret = "";
    if (!this.moveSol) {
      return ret;
    }
    ...
    if (urf < 3) {
      for (let s = 0; s < this.moveSol.length; ++s) {
        ret += Util.MOVE2STR[CubieCube.URFMove[urf][this.moveSol[s]]] + " ";//ĳ����������
      }
    } else {
